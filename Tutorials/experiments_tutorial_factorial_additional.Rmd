...
This results in a table of potential outcomes as produced below, where black values ($Y_i(w_{[1]}, w_{[2]})$) are the ones we observe, and grayed-out values (<font color="lightgray">$Y_i(w_{[1]}, w_{[2]})$</font>) are the ones we don’t observe.
...
### Leaving Interaction Cells Empty

Factorial designs are often used for their power gain relative to conducting separate experiments. A full factorial design explained in previous sections assign participants to each possible combination of levels of the factors. When the main effects (`Average Marginal Effect') of individual factors are of interest, rather than the interaction effects, an alternative design-based approach is to leave the `interaction cell' empty and reassign subjects to other cells. This approach has been explained in more detailed in [Muralidharan et al., (2021)](https://econweb.ucsd.edu/~kamurali/papers/Working%20Papers/CrossCuts%20(Current%20WP).pdf). 

Thus, a simple 2 by 2 factorial design where interaction cell is left empty can be visualized below:
...
Here, the cell value represents the number of participants assigned to that cell or condition. In other words, there is no participant assigned to treatment condition (1,1).

Note that this design allows one to investigate the main effects by sharing a control group, thus having power gain relative to conducting two seprate experiments.

To enable power calculation for this type of experiment, we tweak the code in sections `lm_interacted_model`, `power_simulated` and `power_simulated_graph` to reflect the change in the assignment.

```{r interact_model_leave_empty}

lm_interacted_model_leave_interaction_empty <- function(N,
                                # probability of treatment assignment, 
                                # separate entry for each factor, 
                                # entries will be normalized to sum to 1
                                q = 0.5, # proportion of participants in factor A experiment (factor B in control) (*)
                                p_list = list(c(0.75, 0.25),
                                              c(0.25, 0.5, 0.25),
                                              c(0.25, 0.75)), 
                                # intercept (potential outcome at all baseline)
  ...
  # leave interaction cell empty, randomization within control of the other factor
  # do randomization separately
  #  (*)
  wmat_A <-as.data.frame(
    factor(complete_ra(N*q,  # change the proportion for your needs 
                  num_arms = L_list[[1]], 
                  prob_each = p_list[[1]]),
      levels = paste0('T', 1:L_list[[1]]),
      labels = 1:L_list[[1]]))
  names(wmat_A)[1] <- 'T1'
  wmat_A$T2 <- rep(1, N*q)

  wmat_B <-as.data.frame(
    factor(complete_ra(N*(1-q), 
                  num_arms = L_list[[2]], 
                  prob_each = p_list[[2]]),
      levels = paste0('T', 1:L_list[[2]]),
      labels = 1:L_list[[2]]))
  names(wmat_B)[1] <- 'T2'
  wmat_B$T1 <- rep(1, N*(1-q))

  wmat <- rbind(wmat_A, wmat_B)

  # add names for the treatment assignment  
  colnames(wmat) <- paste0('w', 1:K, '.')
  
  # simulation based on provided data generation process 
  mmat <- model.matrix(formula(paste0('~.^', degree)) , as.data.frame(wmat))[, 1:3] # indicators for whether to include w1, w2 and the interaction
  betas <- unlist(c(b0, b_main_list)) # define the betas
  y <- as.vector(mmat %*% betas + rnorm(N, sd = sigma)) # simulate outcomes
...
```{r power_leave_int_empty}

possible.ns <- seq(from = 500, to = 5000, by = 100) # possible total sample size
#power.interaction <- rep(NA, length(possible.ns)) # save power to detect the specified interaction effect
power.onemarginal <- rep(NA, length(possible.ns)) # save power to detect at least one of the marginal effects
power.allmarginals <- rep(NA, length(possible.ns)) # save power to detect all marginal effects, with Bonferroni correction
power.all_FDR <- rep(NA, length(possible.ns)) # save power to detect all marginal effects, with FDR control
...
for (j in 1:length(possible.ns)) {
  ...
  for (i in 1:sims) {
    ...
    # apply the analysis function defined above
    fits <- lm_interacted_model_leave_interaction_empty(N,
                                q = 0.5,
                                p_list = p_list, # randomization probs
                                b0 = b0, # intercept
                                b_main_list = b_main_list, # main effects
                                sigma = sigma)
    #fit0 <- fits[[1]] # long model with interaction
    fit1 <- fits # short model without interaction # TODO: not used not, check back see if needed
